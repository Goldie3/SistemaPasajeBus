...
const revokeAllSesiones = asyncHandler(async (req, res) => {
  ...
  if (req.query.except === 'current' && req.body.refreshToken) {
    exceptSessionId = await authService.getCurrentSessionIdFromToken(req.body.refreshToken);
  }
...
const forgotPassword = asyncHandler(async (req, res) => {
  ...
  const expiraEn = new Date(Date.now() + 1000 * 60 * 60);

  await db.PasswordResetToken.create({ usuarioId: usuario.id, token, expiraEn });
...
const resetPassword = asyncHandler(async (req, res) => {
  ...
  const resetToken = await db.PasswordResetToken.findOne({
    where: { token, usado: false, expiraEn: { [Op.gt]: new Date() } },
